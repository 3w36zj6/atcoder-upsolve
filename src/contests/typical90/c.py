...
N = int(input())
AB: list[list[int]] = [list(map(int, input().split())) for _ in range(N - 1)]
...
# 直径を求める
graph: list[list[tuple[int, float | int]]] = [[] for _ in range(N)]
...
# 0から一番遠い頂点
dist, parents = dijkstra(graph, 0)
far_vertex_index = dist.index(max(dist))

# 一番遠い頂点から一番遠い頂点
dist, parents = dijkstra(graph, far_vertex_index)
